...
class BinarySearchTree:
    ...
    def dfs_post_order(self):
        results = []
        self._dfs_post_order(self.root,results)
        return results
    
    def _dfs_post_order(self,node,results):
        
        if node.left is not None:
            self._dfs_post_order(node.left,results)
        if node.right is not None:
            self._dfs_post_order(node.right , results)
